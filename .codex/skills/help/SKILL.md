...
#### 스킬 (Skills)
`.codex/skills/*/SKILL.md` 파일에서 name과 description을 추출하여 테이블로 출력.
...
커맨드:
  ...
  [custom] /tm-init    — task graph 초기화
  [custom] /tm-next    — 다음 실행 task 선택
  [custom] /tm-start   — 선택 task 시작
  [custom] /tm-sync    — 세션 상태 동기화
  [custom] /tm-done    — task 완료 반영
  [custom] /tm-board   — task graph 보드 요약
...
Task Master형 확장이 감지되면 아래 항목을 추가로 표시합니다.

- task graph 경로: `.codex/project/taskmaster/`
- 스키마 파일 존재 여부
- 템플릿 파일 존재 여부
- 초기화 스크립트: `.codex/commands/scripts/tm-init.sh`
- 검증 스크립트: `.codex/commands/scripts/tm-validate.sh`
- 부트스트랩 스크립트: `.codex/commands/scripts/tm-bootstrap.sh`
- 현재 `currentTaskId`
- 사용 가능한 command convention (`/tm-next`, `/tm-start`, `/tm-done` 등)

Task Master 확장이 없으면 이를 오류로 간주하지 않고 "비활성"으로 표시합니다.
