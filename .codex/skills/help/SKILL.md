...
`.codex/project/usage-data/` 디렉터리가 존재하면 사용 통계를 함께 표시합니다.

각 카테고리(skills, commands, roles) 하위 파일을 읽어 count를 추출합니다.
