---
name: evolve
description: 프로젝트 변화를 감지하고 .codex 설정을 진화시킵니다. 새로운 의존성, 아키텍처 패턴 변경, 코드베이스 성장에 맞춰 rules와 context를 업데이트합니다. Use when project has changed significantly since last setup/evolve.
...
프로젝트 변화를 감지하고 .codex 설정을 진화시킵니다. 새로운 의존성, 아키텍처 패턴 변경, 코드베이스 성장에 맞춰 rules와 context를 업데이트합니다.

## 진입 조건 (Entry Condition)

`.codex/project/manifest.json`이 존재해야 합니다. setup이 먼저 실행되어야 하며, 없다면 /setup을 먼저 실행하도록 안내합니다.
...
- 무엇이 변경되었는지
- .codex/project/*.md에 대한 제안 업데이트
- 추가할 새 규칙
- 제거할 규칙
- AGENTS.md에 정의된 역할 커스터마이즈 업데이트
...
`.codex/project/history/vN.M-evolution-YYYYMMDD.md` 작성:
