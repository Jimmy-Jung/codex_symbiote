...
```
.codex/project/state/{ISO8601-basic}_{task-name}/
...
경로: `.codex/project/state/{task-folder}/ralph-state.md`
...
### Step 1: 초기화
- task-folder 생성: `mkdir -p .codex/project/state/{ISO8601-basic}_{task-name}`
...
- Task Master 연동 시 `.codex/project/taskmaster/state.json`에서 `currentTaskId`를 확인하고, 있으면 `ralph-state.md`의 `taskId`에 기록
- Task Master 연동 시 전역 상태 파일 경로를 `taskGraphPath`에 기록

### Step 2: ANALYZE
AGENTS.md에 정의된 Analyst 역할로 요구사항 정제, 코드베이스/제약사항 분석. PRD가 있으면 acceptance criteria 로드.
Task Master 연동 시 선택된 `taskId`의 details, dependencies, testStrategy도 함께 로드.

### Step 3: PLAN
planning 스킬에 따라 계획 수립. Grep+SemanticSearch+Glob 병렬 실행하여 코드베이스 분석. AGENTS.md에 정의된 Critic 역할로 계획 검증: 완전성, 의존성, 파괴적 변경, 실현 가능성.

### Step 4: EXECUTE
context.md의 프로젝트 패턴을 따르며 구현. 독립 작업은 병렬 실행(최대 4개). 단계 완료 시마다 TODO 기록.

### Step 5: VERIFY
코드 품질, 패턴 준수, ReadLints, 완료 기준 검증. verify-loop 4-Level 기준 대조.
- Level ≥ 3: AGENTS.md에 정의된 Reviewer 역할로 기능 검증
- Level ≥ 4: security-review, documentation 스킬 추가 검증
...
Step 4: 독립 파일 생성 병렬
Step 5: 여러 파일 ReadLints 병렬, Reviewer 역할 병렬
...
- Task Master 연동 시 `done|review|blocked` 후보 상태를 별도 요약으로 남긴다.
