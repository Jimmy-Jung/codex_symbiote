...
- 모든 프로젝트에서 `state/{task-folder}/task.json`을 필수 상태 파일로 강제
...
- 확장:
  - Task Master 계열 스킬
  - 전역 상태(state/config) + task 템플릿 파일
...
- `prd.json`: 요구사항 입력 계층
- `state/{task-folder}/task.json`: 실행용 task graph 계층
...
### 6.1 task.json (`state/{task-folder}`)
...
- `task.json`은 `prd.json`의 대체물이 아니라 실행용 정규화 결과입니다.
...
- `task.json` 변환을 위한 정규화 규칙 섹션 추가
- `implementedIn` 외에 `taskIds` 연결 필드 추가

목적:

- `prd.json`에서 `task.json`으로 손실 적은 변환 가능
...
- 종료 단계에서 `state/{task-folder}/task.json` status 반영을 위한 sync 규칙 추가
- PRD 직접 로드 외에 `task.json` 기반 실행 경로 추가
...
- Symbiote:
  ...
  - 세션 실행 오케스트레이션
- Task Master형 확장:
  - 작업별 실행 그래프(`state/{task-folder}/task.json`)
  - 전역 상태 포인터(`state.json`)와 정책(`config.json`)
  - dependency 기반 스케줄링
