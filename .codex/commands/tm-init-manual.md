...
1. `.codex/project/taskmaster/state.template.json`을 기준으로 `state.json` 생성
2. `.codex/project/taskmaster/config.template.json`을 기준으로 `config.json` 생성
3. 필요 시 `state.json.currentTag`, `config.json.defaults` 값을 프로젝트 상황에 맞게 수정
4. `bash .codex/skills/doctor/scripts/validate.sh`로 runtime 파일 존재 여부 확인
...
- task 파일은 전역 `tasks.json`이 아니라 `.codex/project/state/{task-folder}/task.json`을 사용한다
