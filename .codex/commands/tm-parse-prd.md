...
2. `userStories[]`를 top-level task 또는 subtask 후보로 해석한다
3. `dependsOn[]`를 `dependencies[]`로 변환한다
4. `acceptanceCriteria[]`를 `testStrategy` 또는 subtask 검증 항목으로 변환한다
5. `taskIds[]`를 역참조 키로 기록한다
