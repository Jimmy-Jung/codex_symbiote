...
- `.agents/skills/note/SKILL.md` — 타임아웃 시 상태 보존
- `.agents/skills/verify-loop/SKILL.md` — 자율 루프에서 에스컬레이션
...
- `.agents/skills/notify-user/.env` 파일이 본인의 Slack 정보로 설정되어 있어야 합니다
- 최초 설정 시 SETUP-GUIDE.md를 참고하세요

## 환경 변수 (.env)

이 스킬은 `.agents/skills/notify-user/.env` 파일에서 사용자별 Slack 정보를 읽습니다.
최초 사용 시 `.env.example`을 복사하여 `.env`로 만들고 본인의 정보를 입력하세요.

```bash
cp .agents/skills/notify-user/.env.example .agents/skills/notify-user/.env
```
...
```
1. Read tool로 .agents/skills/notify-user/.env 파일을 읽는다
...
```
1. .codex/project/state/*/ralph-state.md 파일을 확인한다
